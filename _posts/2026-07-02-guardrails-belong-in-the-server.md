...
description: >-
  ...
  returns, and output scanning, all deterministic code between the model and the
  API. With the allowlist bug that re-opens the doors you closed.
date: 2026-07-02
...
model is the one component in your system you have to assume can be turned
against you. A poisoned tool result, an injected instruction buried in a fetched
document, a cleverly worded user message: any of these can make the model
*want* to call the tool you told it not to. This isn't hypothetical. Researchers
like [Pliny the Liberator](https://pliny.gg) reliably jailbreak frontier models
within *hours* of release, so assume yours is next. If the only thing standing
between a hijacked model and your API is another sentence in the same prompt the
attacker just rewrote, you have no control at all.

**Guardrails have to live in the server**, in deterministic code that runs
between the model's decision and the actual side effect and doesn't care what
the model was convinced to do. Here are the three that matter most.
...
before it executes. Not documented. Enforced. A call to anything not on the list
is rejected in code and logged as a security event.
...
you can watch get bypassed and then fixed.
