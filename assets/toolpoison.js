...
(function () {
	...
	var state = { rung: "L0", running: false };
	...
	function renderVerdict(d) {
		...
		if (d.leaked) {
			v.className = "tp-verdict tp-verdict-leaked";
			v.textContent = "Leaked. Your description reached the model and steered it into reading the account note — the token left the boundary.";
		} else {
			v.className = "tp-verdict tp-verdict-held";
			if (action === "withheld") {
				v.textContent = "Held. The content scan matched an injection marker and withheld your description. (Plainer wording can still slip past it — try the bored one.)";
			} else if (action === "reverted") {
				v.textContent = "Held. The descriptor pin reverted your edit to the approved text by hash, so your wording never reached the model. This is the control that doesn't care how the injection is phrased.";
			} else {
				v.textContent = "Held this run. The description was served, but the agent didn't take the bait — a live model is probabilistic. Try again, or sharpen the wording.";
			}
		}
	...
	function loadStats() {
		fetch(API + "/stats").then(function (r) { return r.json(); }).then(function (d) {
			if (!d || !d.tp) return;
			var ex = d.tp.exfils || {}, at = d.tp.attempts || {};
			clearNode(els.board);
			["L0", "L2", "L3"].forEach(function (rung) {
				els.board.appendChild(boardCard(rung, ex[rung], at[rung]));
			});
